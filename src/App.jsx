...
export default function App() {
  ...
  if (active === 'Feedback') return <><Header active={active} onNavigate={navigate} onEmergency={emergency} /><main className="page-content feedback-page"><div className="section-intro"><div><div className="eyebrow">TOURSAFE FEEDBACK</div><h1>Help us make<br /><em>travel safer.</em></h1><p className="muted">Report incorrect safety information or share feedback about your route experience.</p></div></div><section className="feedback-card feedback-session"><div><div className="eyebrow">REPORT AN ISSUE</div><h2>Send feedback</h2><p>Your feedback will be addressed to <b>yogeshwari1923@gmail.com</b>.</p></div><form className="feedback-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const name = String(form.get('name') || '').trim(); const email = String(form.get('email') || '').trim(); const feedbackText = String(form.get('feedback') || '').trim(); if (!name || !email || !feedbackText) return setNotice('Name, email, and feedback are required.'); if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return setNotice('Please enter a valid email or Gmail address.'); const subject = `TourSafe feedback from ${name}`; const body = `Name: ${name}\nEmail: ${email}\n\nFeedback:\n${feedbackText}`; window.location.href = `mailto:yogeshwari1923@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}` }}><div className="feedback-fields"><input name="name" type="text" placeholder="Your name" required /><input name="email" type="email" placeholder="Email / Gmail address" required /><textarea name="feedback" placeholder="Describe the issue or feedback…" rows="5" required /></div><button className="primary-button" type="submit"><Info size={14} /> Submit feedback</button></form>{notice && <div className="notice"><Info size={16} /> {notice}</div>}</section></main></>
